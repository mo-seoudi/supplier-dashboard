...
// ---- helpers for AY / YTD / YoY ----

/**
 * Generic groupBy that’s safe for TS: K must be a PropertyKey (string|number|symbol).
 * Avoids "||=" and avoids 'any' casts.
 */
...
export function totalsByMonth(rows: SalesRow[]) {
  const byMonth = groupByKey(rows, (r) => r.month as PeriodKey); // Record<PeriodKey, SalesRow[]>
...
export function ytdTotals(rows: SalesRow[], ay: string, ayStartMonth = 9) {
  ...
  if (months.length === 0)
    return { actualSales: 0, forecastSales: 0, lastMonth: null as PeriodKey | null };
  let a = 0, f = 0;
...
export function yoyVsPrevAY(rows: SalesRow[], ay: string, ayStartMonth = 9) {
  const [y1] = ay.split("-").map((x) => Number(x));
