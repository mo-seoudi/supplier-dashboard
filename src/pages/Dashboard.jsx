 ...
 * Expects /public/data/sales_tidy.json with rows:
 * { category, date: "YYYY-MM-DD", school, metric: "Sales"|"Commission"|"Sales Forecast"|"Commission Forecast", value }
 */
...
// --- Tiny UI helpers (native selects + simple tabs) -------------------------
function LabeledSelect({ label, value, onChange, options }) {
  return (
    <div>
      <label className="text-xs text-slate-500">{label}</label>
      <select
        className="w-full mt-1 rounded-2xl border px-3 py-2 bg-white"
        value={value}
    ...
    <div className="grid grid-cols-3 gap-2 w-full sm:w-auto">
      ...
      >
        <span className="inline-flex items-center gap-2">
          <LineChartIcon className="h-4 w-4" /> Trend
        </span>
      </button>
      <button
        className={`${base} ${tab === "leaders" ? active : idle}`}
        onClick={() => setTab("leaders")}
      >
        <span className="inline-flex items-center gap-2">
          <BarChart3 className="h-4 w-4" /> Leaders
        </span>
      </button>
      <button
        className={`${base} ${tab === "mix" ? active : idle}`}
        onClick={() => setTab("mix")}
      >
        <span className="inline-flex items-center gap-2">
          <PieChartIcon className="h-4 w-4" /> Mix
        </span>
      </button>
    </div>
  );
...
export default function Dashboard() {
  ...
  // Fetch data from /public/data/sales_tidy.json, fallback to inline sample
  useEffect(() => {
    (async () => {
      try {
  ...
  const schools = useMemo(
    () => ["All", ...Array.from(new Set(rows.map((r) => r.school))).sort()],
    [rows]
  );
  ...
  // Series: totals by school (bar chart)
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">
              Interactive dashboard generated from your Excel structure.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() => window.open("/data/sales_tidy.json", "_blank")}
            >
            ...
            <CardContent className="p-5">
              ...
              <div
                className={`text-3xl font-semibold mt-1 ${
                  kpis.changeMoM >= 0 ? "text-emerald-600" : "text-rose-600"
                }`}
              >
        ...
        {tab === "leaders" && (
          <Card className="shadow-sm mt-4">
            <CardContent className="p-4">
              <div className="text-sm text-slate-600 mb-2">
                Top schools by total {metric}
              </div>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={bySchool.slice(0, 15)}
                    margin={{ top: 8, right: 12, left: 0, bottom: 8 }}
                  >
                    <XAxis
                      dataKey="school"
        ...
        {tab === "mix" && (
          <Card className="shadow-sm mt-4">
            <CardContent className="p-4">
              <div className="text-sm text-slate-600 mb-2">
                {category === "All" ? "Category mix" : `${category} mix by school`}
              </div>
                        ...
                        const series = (() => {
                          if (category === "All") {
                            ...
                            rows
                              .filter(
                                (r) =>
                                  r.metric === metric && r.category === category
                              )
                              .forEach((r) =>
                                m.set(r.school, (m.get(r.school) || 0) + r.value)
                              );
                            return Array.from(m.entries()).map(([name, value]) => ({
                              name,
                              value,
        ...
        <footer className="text-xs text-slate-500 pt-6">
          Built with React + Tailwind + your UI primitives + Recharts.
          Place <code>sales_tidy.json</code> in <code>public/data/</code>.
        </footer>
      </div>
    </div>
  );
