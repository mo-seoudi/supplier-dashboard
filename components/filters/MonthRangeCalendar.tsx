...
export default function MonthRangeCalendar({ ay }:{ ay: string }) {
  ...
  const months = useMemo(()=> monthsForAY(ay), [ay]);
  ...
  function handlePick(m: string) {
    if (!start || (start && end)) { setRange(m, m); }
    else {
      const sIdx = months.indexOf(start); const mIdx = months.indexOf(m);
      if (sIdx === -1 || mIdx === -1) return;
      if (mIdx < sIdx) setRange(m, start); else setRange(start, m);
    }
  }

  function clear() { setRange("", ""); }

  const sIdx = months.indexOf(start); const eIdx = months.indexOf(end);
  ...
  return (
    <div className="text-sm">
        ...
        {months.map((m, i) => {
          ...
          return (
            <button key={m} onClick={() => handlePick(m)}
              className={`rounded-md border px-2 py-1 text-left ${selected ? "bg-gray-900 text-white border-gray-900" : "hover:bg-gray-100"}`}>
              {m}
            </button>
          );
        })}
      ...
      <div className="mt-2 flex gap-2">
        <button type="button" onClick={clear} className="text-xs underline text-gray-600">Clear</button>
