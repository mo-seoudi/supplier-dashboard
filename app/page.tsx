...
export default function Page({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  ...
  // Data for KPIs / Monthly chart: apply all filters
  ...
  const monthly = Array.from(monthlyMap.entries()).sort(([a],[b]) => a < b ? -1 : 1).map(([month, actual]) => ({ month, actual }));

  // Compare Schools chart: ignore School filter, but respect AY, month range, income type
  ...
  for (const r of compareSlice) {
    ...
    o.val += r.actualSales; bySchool.set(r.schoolId, o);
  }
  const compareArr = Array.from(bySchool.values()).sort((a,b)=> b.val - a.val).slice(0, 12).map(o => ({ school: o.name, actual: o.val }));
